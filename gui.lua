...
function ButtonMethods:Update(settings)
    if settings.Name then
        self.Name = settings.Name
        self._instance.Text = settings.Name
    end
...
function ToggleMethods:Update(settings, triggerCallback)
    ...
    if settings.State ~= nil then
        self.Value = settings.State
    end
    if settings.Callback then
        self.Callback = settings.Callback
    end

    self._instance.Text = self.Name .. ": " .. (self.Value and "On" or "Off")
...
function InputMethods:Update(settings, triggerCallback)
    triggerCallback = triggerCallback == nil and true or triggerCallback

    if settings.Name then
        self.Name = settings.Name
        self._instance.Text = settings.Name
        self.Text = settings.Name
    end
    if settings.Callback then
        self.Callback = settings.Callback
    end
...
setmetatable(ParagraphMethods, {__index = BaseMethods})
function ParagraphMethods:Update(settings)
    if settings.Name then
        self.Name = settings.Name
        local titleLabel = self._instance:FindFirstChild("Title")
        if titleLabel then titleLabel.Text = settings.Name end
    end
    if settings.Content then
        self.Content = settings.Content
        local contentLabel = self._instance:FindFirstChild("Content")
        if contentLabel then contentLabel.Text = settings.Content end
    end
end
...
function ContainerMethods:CreateInput(settings)
    ...
    input:Bind(input._instance.FocusLost, function(enterPressed)
        ...
        if enterPressed and input.Callback then input.Callback(input.Text) end
    end)
...
function ContainerMethods:CreateParagraph(settings)
    local paragraph = {
        Name = settings.Name or "Paragraph",
    ...
    setmetatable(paragraph, {__index = ParagraphMethods})

    paragraph._instance = initialInstance({
        Instance = "Frame", Parent = self._frame,
        Settings = {
            Name = randStr(10),
    ...
    initialInstance({
        Instance = "UIListLayout", Parent = paragraph._instance,
        Settings = {
            Name = randStr(10),
    ...
    initialInstance({
        Instance = "UIPadding", Parent = paragraph._instance,
        Settings = {
            Name = randStr(10),
    ...
    initialInstance({
        Instance = "TextLabel", Parent = paragraph._instance,
        Settings = {
            Name = "Title",
            Text = paragraph.Name,
    ...
    self.Children[settings.Name] = paragraph
    return paragraph
end
...
function SimpleGui:CreateWindow(windowSettings)
    ...
    window._instance = initialInstance({
    ...
    local backButton = initialInstance({Instance = "TextButton", Parent = topBar, Settings = {
        Size = UDim2.new(0, 25, 1, 0), Position = UDim2.new(0, 0, 0, 0), Text = "<", BackgroundTransparency = 1, Visible = false, ZIndex = 3
    }})
